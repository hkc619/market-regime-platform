...
def build_dataset(feat_clean, labels_clean, regime_clean, split_tr, split_val):
    ...
    Xs_s, Xs_m, y_seq, r_seq = build_dual_sequences(
        feat_scaled_all, label_arr, regime_arr, seq_len_s, seq_len_m) # SEQ_LEN_S, SEQ_LEN_M

    # Chronological boundaries in sequence space (offset by SEQ_LEN_M warmup)
    seq_split_tr  = split_tr  - seq_len_m # SEQ_LEN_M
    seq_split_val = split_val - seq_len_m # SEQ_LEN_M
