...
def label_generate(feat, adx_aligned, adx_regime, trend_fast, trend_slow, di_bull, idx, train_frac, val_frac):
    ...
    def clean_dataset(labels_raw):
        ...
        print("lg valid_index: ", valid_idx)
        feat_clean   = feat.loc[valid_idx]
        labels_clean = labels.loc[valid_idx]
        regime_clean = adx_regime.loc[valid_idx]

        return valid_idx, feat_clean, labels_clean, regime_clean

    valid_idx, feat_clean, labels_clean, regime_clean = clean_dataset(labels_raw)
...
def generate_predict_label(feat, adx_aligned, adx_regime, trend_fast, trend_slow, di_bull, idx):
    fast_above_slow = (trend_fast > trend_slow).reindex(idx).astype(int)
    adx_strong      = (adx_aligned > 20).astype(int)
    ...
    return feat_clean, regime_clean